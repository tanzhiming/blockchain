...
链码是一个程序，它由Go，node.js或者Java编写，实现了规定的接口。链码运行在安全的Docker容器中，与peer进程隔离。链码通过应用程序的事务提交来初始化和管理账本状态。

链码典型的处理网络成员同意的业务逻辑，因此它类似“智能合约”。在提案交易中调用链码来更新或者查询账本。给予适当权限，一个链码可以调用另一个链码（可以是相同通道或者不同通道）来访问它的状态。注意，如果被调用的链码位于与调用链码不同的通道上，则只允许查询。也就是说，在不同通道上调用的链码只是一个`Query`，它不参与后续提交阶段的状态验证检查。

## Chaincode API


[Chaincode 接口](https://godoc.org/github.com/hyperledger/fabric/core/chaincode/shim#Chaincode)
...
1. 当chaincode接收到`instantiate`和`upgrade`交易时`init`方法被执行
2. 当接收到`invoke`交易时调用`invoke`方法来处理交易提案

[Chaincode "shim" 接口](https://godoc.org/github.com/hyperledger/fabric/core/chaincode/shim#ChaincodeStubInterface)
...
>注意：链码升级也会调用`init`方法，升级一个存在的链码时，确保修改`init`方法。如果不涉及数据迁移等情况，可以提供一个空的`init`方法。


使用`ChaincodeStubInterface.GetStringArgs`方法提取参数，验证参数合法性
...
调用`ChaincodeStubInterface.PutState`初始化状态到账本
...
使用 ChaincodeStubInterface.GetFunctionAndParameters来获取应用程序方法和参数。我们应用程序包括`set`和`get`两个方法。
...
使用`ChaincodeStubInterface.PutState`和`ChaincodeStubInterface.GetState`来访问账本
...
最后，我们需要增加一个`main`方法，它调用`shim.Start`方法。
...
## 安装Hyperledger Fabric 样例

首先确保已经安装了`fabric-samples`, 导航到`chaincode-docker-devmode`

```shell
cd chaincode-docker-devmode
```
现在打开三个终端并且都切换到`chaincode-docker-devmode`
